...
// Assinatura simples — funciona com a conta SmartLife vinculada
async function getToken() {
  ...
  if (!data.success) throw new Error('Auth: ' + JSON.stringify(data));
...
app.get('/', (req, res) => res.json({ status: 'FTM Smart API online', version: '5.0.0' }));
...
app.get('/api/homes/:homeId/devices', async (req, res) => {
  try {
    ...
    let devices = [];
    try {
      const r = await tuyaGet(token, `/v1.0/homes/${req.params.homeId}/devices`);
...
app.listen(PORT, () => console.log(`FTM Smart v5 porta ${PORT}`));
