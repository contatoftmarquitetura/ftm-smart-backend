...
async function api(tok, method, path, body) {
  ...
  const opts = {
    method,
    headers: { 'client_id': CLIENT_ID, 'access_token': tok, 't': t, 'sign': s, 'sign_method': 'HMAC-SHA256', 'Content-Type': 'application/json' }
  };
...
app.get('/', (req, res) => res.json({ status: 'FTM Smart API online', version: '10.0.0' }));

app.get('/api/debug', async (req, res) => {
  try {
    const tok = await token();
    const devices = await api(tok, 'GET', '/v1.0/iot-01/associated-users/devices?last_row_key=');
    res.json({ token_ok: true, devices_success: devices.success, devices_count: devices.result?.devices?.length || 0, raw: devices });
  } catch(e) {
...
app.get('/api/devices', async (req, res) => {
  try {
    ...
    if (r.success && r.result && r.result.devices) {
      devices = r.result.devices;
    }
    res.json({ success: true, result: { devices: devices }, count: devices.length });
  } catch(e) {
...
app.get('/api/homes', async (req, res) => {
  try {
    ...
    res.json({ success: true, result: { homes: homes }, count: homes.length });
  } catch(e) {
    res.status(500).json({ success: false, error: e.message });
  }
});
...
app.get('/api/homes/:id/devices', async (req, res) => {
  try {
    ...
    res.json({ success: true, result: { devices: devices } });
  } catch(e) {
    res.status(500).json({ success: false, error: e.message });
  }
});
...
app.listen(PORT, function() { console.log('FTM Smart v10 porta ' + PORT); });
