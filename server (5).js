...
app.get('/', (req, res) => {
  res.json({ status: 'FTM Smart API online', version: '5.0.0-restored' });
});
...
app.get('/api/myip', async (req, res) => {
  try {
    ...
    res.json({ server_ip: d.ip, message: 'Este é o IP do servidor Render — adicione na whitelist Tuya' });
  } catch(e) {
...
app.get('/api/homes/:id/devices', async (req, res) => {
  try {
    const token = await getToken();
    const r = await tuyaRequest(token, 'GET', `/v1.0/homes/${req.params.id}/devices`);
...
app.listen(PORT, () => console.log(`FTM Smart v5-restored porta ${PORT}`));
